package ac 
{
	...
	public class  testAction extends MovieClip
	{
		...
		private var progressData:Number=0;
		private var GetScore:Number=0;   
		...
		public function getDat(num:Number,obj:Object,score:Number):void
		{
			progressData=num;
			GetScore=score;
			myObj=obj;
			
			trace(progressData+"---------"+GetScore+"--------------");
		}
		...
		private function choose_BtnDown(e:MouseEvent):void
		{
			...
			if(e.target.tipMc.currentFrame==1)
			{
				e.target.tipMc.gotoAndStop(2);
			}else  if(e.target.tipMc.currentFrame==2)
			{
				
				e.target.tipMc.gotoAndStop(1);
			}
			...
			if(btnDownBoo==true)
			{
				...
				}else
				{
					...
					endpage.score.text=rightINT*10+"分。";
					 // ExternalInterface.call("alert","加载的分数："+GetScore+"上传的分数:"+rightINT*10);
					if(GetScore<rightINT*10)
					{
						// ExternalInterface.call("alert","分数上传:"+String(rightINT*10));
					    ExternalInterface.call("setScore",String(rightINT*10));
					...
					if(rightINT>=8)
					{
						...
						if(progressData>=50)
						{
							...
							//ExternalInterface.call("alert","课程已完成"+progressData);
							trace("SetComplete");
						}else
						{
							
							ExternalInterface.call("SetPassed");
							//ExternalInterface.call("alert","课程合格"+progressData);
							trace("SetPassed");
						}
